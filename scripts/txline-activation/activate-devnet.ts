 ...
 *   3. Acquire a guest JWT from POST https://txline-dev.txodds.com/auth/guest/start.
 *   4. Sign the message `${txSig}:${leagues.join(",")}:${jwt}` with the wallet
 *      key (ed25519 detached, base64).
 *   5. POST https://txline-dev.txodds.com/api/token/activate → returns the API token.
 *
...
async function main() {
  ...
  if (sol < MIN_SOL) {
    console.error(
      `Need at least ${MIN_SOL} devnet SOL to activate. Fund the wallet above and re-run.`,
    );
    process.exit(1);
  }
